...
export async function connectConvaiClean() {
  ...
  try {
    const wsUrl = await getSignedUrl(import.meta.env.VITE_EXCELSIOR_AGENT_ID);
    ...
    ws.onopen = async () => {
      ...
      processor.connect(audioCtx.destination); // imprescindible para que corra onaudioprocess

      processor.onaudioprocess = (e) => {
        if (!ws || ws.readyState !== WebSocket.OPEN) return;
        const inBuf = e.inputBuffer.getChannelData(0);
        const level = rms(inBuf);
        if (level <= 0.01) return; // ignora ruido bajo
        const ds = downsampleTo16k(inBuf, audioCtx!.sampleRate);
        const pcm = floatToPcm16le(ds);
        const b64 = u8ToB64(pcm);
        try {
          ws.send(JSON.stringify({ user_audio_chunk: b64 }));
          console.log('[WS ->] user_audio_chunk', { bytes: pcm.byteLength });
        } catch (err) {
          console.warn('[convai-clean] send user_audio_chunk error', err);
        }
      };
    };
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg?.type === 'ping' && msg?.ping_event?.event_id) {
        try {
          ...
          console.log('[convai-clean] pong sent');
        } catch {}
      }

      // inicio de respuesta del agente (para logs)
      if (msg?.type === 'agent_response') {
        console.log('[convai-clean] agent_response → incoming TTS');
      }

      // audio del agente: programar en serie (cola)
      if (msg?.type === 'audio' && msg?.audio_event?.audio_base_64) {
        ...
        // debounce corto para marcar fin de respuesta si no llega response.completed
        if (silenceTimer) { clearTimeout(silenceTimer as any); }
        silenceTimer = window.setTimeout(() => {
          const since = performance.now() - lastAudioAt;
          if (since > 250) {
            // fin de respuesta por silencio: solo resetea la cola
      ...
      // fin explícito de respuesta (si el agente lo manda)
...
// @ts-ignore
(window as any).connectConvaiClean = connectConvaiClean;
// @ts-ignore
(window as any).disconnectConvaiClean = disconnectConvaiClean;
